...
/***************************创建imap邮箱相关表******************************/
+(BOOL)createMCOFolder
{
    NSString* t_folder_sql = [NSString stringWithFormat:@"create table if not exists %@\
                           (\
                           ID 				INTEGER PRIMARY KEY AUTOINCREMENT,\
    ...
    char *error = NULL;
    [self openDb];
    if (sqlite3_exec(dataBase, [t_folder_sql UTF8String], 0, 0, &error) != SQLITE_OK) {
        NSLog(@"create table %@ error:%s",@"T_DOCUMENTS",error);
        [self closeDb];
        return NO;
    }
