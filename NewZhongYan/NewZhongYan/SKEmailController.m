...
-(void)getNewEmailFromServerCompleteBlock:(basicBlock)block
{
    ...
    /* test imap */
//    NSMutableString *username = [NSMutableString stringWithFormat:@"%@%@", [[APPUtils loggedUser] uid],@"@hngytobacco.com"];
//    MCOIMAPSession *session = [[MCOIMAPSession alloc] init];
//    [session setHostname:IMAP_HOST_NAME];
//    [session setPort:IMAP_PORT];
//    [session setUsername:username];
//    [session setPassword:[[APPUtils loggedUser] password]];
//    [session setConnectionType:MCOConnectionTypeClear];
//    
//    NSLog(@"username = %@", username);
//    NSLog(@"password = %@", [[APPUtils loggedUser] password]);
//    
...
//            NSLog(@"imapFolderName:%@", imapFolder.path);
//            //NSLog(@"canEncode UTF-8:%hhd", [imapFolder.path canBeConvertedToEncoding:NSUTF8StringEncoding]);
//            //NSLog(@"encoding is:%lu", (unsigned long)[imapFolder.path fastestEncoding]);
//            //NSData *folderNameData = [imapFolder.path dataUsingEncoding:NSUTF8StringEncoding];
//            //NSString *imapFolderName = [[NSString alloc] initWithData:folderNameData encoding:NSUTF8StringEncoding];
//            //NSLog(@"imapFolderName:%@", imapFolderName);
//            NSLog(@"folderName:%@", [imapFolder.path imapUTF7Decode]);
//            
//            MCOIMAPFetchMessagesOperation *fetchOperation = [session fetchMessagesByUIDOperationWithFolder:imapFolder.path requestKind:requestKind uids:uids];
//            [fetchOperation start:^(NSError * error, NSArray * fetchedMessages, MCOIndexSet * vanishedMessages) {
//                //We've finished downloading the messages!
//                
//                //Let's check if there was an error:
...
//                NSLog(@"--------------------------------我是分割线Begain----------------------------------");
//                //And, let's print out the messages...
//                NSLog(@"The %@:%@", [imapFolder.path imapUTF7Decode], fetchedMessages);
//                NSLog(@"--------------------------------我是分割线END-------------------------------------");
//            }];
//        }
//        
...
/*
    ...
    [imapCapabilityOP start:^(NSError *error, MCOIndexSet *capabilities) {
        if (error) {
            NSLog(@"Error capabilityOperation:%@", error);
        }
        NSLog(@"capabilityOperation:%@", capabilities);
    }];
    
    //获取文件夹
    MCOIMAPFetchFoldersOperation *imapFolderOP = [[instance imapSession] fetchAllFoldersOperation];
    [imapFolderOP start:^(NSError *error, NSArray *folders) {
        if (error) {
            NSLog(@"Error fetching folder:%@", error);
        }
        for (MCOIMAPFolder *folder in folders) {
            if (![[folder.path imapUTF7Decode] isEqualToString:@"INBOX/xxx1"]) {
                //continue;
            }
            NSLog(@"folder:%@, %@, %c, %u", [folder.path imapUTF7Decode], folder.path, folder.delimiter, folder.flags);
            //获取文件夹信息
            MCOIMAPFolderInfoOperation *imapFolderInfoOP = [[instance imapSession] folderInfoOperation:folder.path];
            [imapFolderInfoOP start:^(NSError *error, MCOIMAPFolderInfo *info) {
                if(error) {
                    NSLog(@"Error fetching folder info:%@", error);
                }
                NSLog(@"%@ folderInfo:%u, %u, %llu, %d", [folder.path imapUTF7Decode], info.uidNext, info.uidValidity, info.modSequenceValue, info.messageCount);
            }];
            
            //获取文件夹状态
            MCOIMAPFolderStatusOperation *imapFolderStatusOP = [[instance imapSession] folderStatusOperation:folder.path];
            [imapFolderStatusOP start:^(NSError *error, MCOIMAPFolderStatus *status) {
                if (error) {
                    NSLog(@"Error fetching folder status:%@", error);
                }
                NSLog(@"%@ folderStatus:%u, %u, %u, %u, %u, %llu", [folder.path imapUTF7Decode], status.uidNext, status.uidValidity, status.recentCount, status.unseenCount, status.messageCount, status.setHighestModSeqValue);
            }];
